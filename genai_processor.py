...
class LanguageModelProcessor:
    ...
    def create_convo_prompt(self, vocab):
        translation_request = (
            f"Proivde a translation into the learner's preferred language ({self.settings["preferred_language"]}). Emphasize clarity and accuracy in your translation. Where relevant, include brief annotations or explanations to highlight cultural or contextual nuances. These insights should elucidate expressions, idioms, or cultural references that may not directly translate but are crucial for understanding the dialogue's deeper meanings and implications."
            if self.settings["translation_on"]
        ...
        mistakes_request = (
            ...
            if self.settings["highlight_mistakes_on"]
            else ""
        )

        prompt = f"""
        "Create a dialogue in {self.settings["practice_language"]}, tailored specifically to the CEFR level {self.settings["learner_level"]}. Your objective is to seamlessly incorporate the target vocabulary word '{vocab}' into a conversation that is relevant to the given theme or context, '{self.settings["conversation_context"]}'. Please adhere to the following guidelines to ensure a high-quality learning experience:

        Scenario Introduction: Begin with a concise description of the scenario in the learner's preferred language, '{self.settings["preferred_language"]}'. This description should be engaging and clear, setting the stage for the dialogue. Briefly outline the setting, characters involved, and the situation they are in, making sure it aligns with the theme/context '{self.settings["conversation_context"]}'.

        Dialogue Construction:

        Compose 3-5 exchanges between characters in {self.settings["practice_language"]}, ensuring the dialogue is realistic and relevant to the learners' experiences.
        ...
        Adjust the dialogue to match the specified {self.settings["practice_language"]} CEFR level '{self.settings["learner_level"]}', considering sentence complexity, vocabulary, and grammatical structures appropriate for that level.
        Formality Register: Ensure the dialogue reflects the requested level of formality ('{self.settings["formality"]}'). This could range from informal, using colloquial language and contractions, to formal, employing polite forms, professional terminology, and complete sentences.

        Dialogue Length and Complexity: Aim for a total word count of approximately 100-150 words for the entire dialogue. This ensures enough {self.settings["practice_language"]} content for educational value without overwhelming the learner. Sentences should vary in length and complexity according to the CEFR level specified.

        Ensure that your {self.settings["practice_language"]} dialogue is not only a learning tool but also a means for reflection and deeper engagement with the language. The goal is to make each dialogue a stepping stone towards fluency, providing learners with practical language skills they can apply in real-world situations.
    ...
    def create_compre_prompt(self, text):
        ...
        level_prompt = {
            ...
            self.settings["learner_level"]
        ) 

        prompt = f"Simplify this text into {level_prompt} for {self.set_settings["practice_language"]} language learners: {text}"
